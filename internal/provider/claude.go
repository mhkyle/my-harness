...
func NewZhipuClaudeProvider(baseURL, apiKey, model string) *ClaudeProvider {
	if apiKey == "" {
		panic("please set ZHIPU_API_KEY environment variable")
	}
	if baseURL == "" {
...
func (p *ClaudeProvider) Generate(ctx context.Context, msgs []schema.Message, availableTools []schema.ToolDefinition) (*schema.Message, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Claude/Zhipu API request failed: %w", err)
	}
